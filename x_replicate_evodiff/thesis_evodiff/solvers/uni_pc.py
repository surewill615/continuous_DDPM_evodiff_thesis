...
class UniPC:
    ...
    def multistep_uni_pc_vary_update(self, x, model_prev_list, t_prev_list, t, order, use_corrector=True):
        ...
        if self.predict_x0:
            ...
            if len(D1s) > 0:
                ...
                for k in range(K - 1):
                    x_t = x_t - alpha_t * h_phi_ks[k + 1] * (D1s * A_p[k].view(1, -1, *([1]*(D1s.dim()-2)))).sum(1)
            # now corrector
            if use_corrector:
                ...
                k = 0
                for k in range(K - 1):
                    x_t = x_t - alpha_t * h_phi_ks[k + 1] * (D1s * A_c[k][:-1].view(1, -1, *([1]*(D1s.dim()-2)))).sum(1)
            ...
            if len(D1s) > 0:
                # compute the residuals for predictor
                for k in range(K - 1):
                    x_t = x_t - sigma_t * h_phi_ks[k + 1] * (D1s * A_p[k].view(1, -1, *([1]*(D1s.dim()-2)))).sum(1)
            # now corrector
            if use_corrector:
                model_t = self.model_fn(x_t, t)
                D1_t = (model_t - model_prev_0)
                ...
                k = 0
                for k in range(K - 1):
                    x_t = x_t - sigma_t * h_phi_ks[k + 1] * (D1s * A_c[k][:-1].view(1, -1, *([1]*(D1s.dim()-2)))).sum(1)
    ...
    def multistep_uni_pc_bh_update(self, x, model_prev_list, t_prev_list, t, order, x_t=None, use_corrector=True):
        ...
        if self.predict_x0:
            ...
            if x_t is None:
                if use_predictor:
                    pred_res = (D1s * rhos_p.view(1, -1, *([1]*(D1s.dim()-2)))).sum(1)
                else:
                    pred_res = 0
            ...
            if use_corrector:
                model_t = self.model_fn(x_t, t)
                if D1s is not None:
                    corr_res = (D1s * rhos_c[:-1].view(1, -1, *([1]*(D1s.dim()-2)))).sum(1)
                else:
                    corr_res = 0
                D1_t = (model_t - model_prev_0)
            ...
            if use_corrector:
                model_t = self.model_fn(x_t, t)
                if D1s is not None:
                    corr_res = torch.einsum('k,bkd->bd', rhos_c[:-1], D1s)
                else:
                    corr_res = 0
                D1_t = (model_t - model_prev_0)
